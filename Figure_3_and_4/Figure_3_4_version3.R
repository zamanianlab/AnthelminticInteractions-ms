...
trans_data <- read_rds(here('data_EGR/iso_data_EGR.rds')) %>% #trans_data <- read_rds('/Users/elenagr/Desktop/iso_data_EGR.rds') %>%
...
synergy <- normalized_data %>% 
  ...
  select(plate, strains, well, contains(c('treatment', 'conc')), norm_DMSO, contains(c('inhibition')), raw_length) %>% 
  mutate(
    conc1 = case_when(
      treatment1 == 'DMSO' ~ 0,
      TRUE ~ conc1),
...
ggsave(here('/Users/elenagr/Desktop/Figure_3.tiff'), Fig_3_combined, width = 300, height = 200, units = 'mm', bg = "white")

ggsave(here('/Users/elenagr/Desktop/Figure_4.tiff'), Fig_4_combined, width = 300, height = 230, units = 'mm', bg = "white")

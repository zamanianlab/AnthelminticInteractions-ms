...
dr_data <- read_rds(here('data_EGR/dr_data_EGR.rds')) %>% #dr_data <- read_rds('/Users/elenagr/Desktop/dr_data_EGR.rds') %>%
  filter(case_when(
...
N2_ec50_norm_DMSO_dyf <- ec50_norm_DMSO_all %>% filter(curve_norm_DMSO == 'N2') %>% select(-genotype) %>% mutate(genotype = '*dyf-2(m160)*')
N2_ec50_norm_DMSO_eat <- ec50_norm_DMSO_all %>% filter(curve_norm_DMSO == 'N2') %>% select(-genotype) %>% mutate(genotype = '*eat-2(ad453)*')
N2_ec50_norm_DMSO_bus <- ec50_norm_DMSO_all %>% filter(curve_norm_DMSO == 'N2') %>% select(-genotype) %>% mutate(genotype = '*bus-5(br19)*')  
N2_ec50_norm_DMSO_agm <- ec50_norm_DMSO_all %>% filter(curve_norm_DMSO == 'N2') %>% select(-genotype) %>% mutate(genotype = '*agmo-1(e3016)*')
N2_ec50_norm_DMSO_nhr <- ec50_norm_DMSO_all %>% filter(curve_norm_DMSO == 'N2') %>% select(-genotype) %>% mutate(genotype = '*nhr-8(ok186)*')
N2_ec50_norm_DMSO_che <- ec50_norm_DMSO_all %>% filter(curve_norm_DMSO == 'N2') %>% select(-genotype) %>% mutate(genotype = '*che-1(p672)*')  
 ...
 (curve_plot_amphid <- revised_data %>%
    ...
    geom_vline(data = N2_ec50_norm_DMSO_che, aes(xintercept = ec50_average), color = '#fac04e', linetype = 'solid', size = 0.3, alpha = 0.75) +
    geom_vline(data = N2_ec50_norm_DMSO_dyf, aes(xintercept = ec50_average), color = '#fac04e', linetype = 'solid', size = 0.3, alpha = 0.75) +
...
#     ggplot() +
#     geom_vline(data = ec50_norm_DMSO %>% filter(curve_norm_DMSO == 'N2'),
#                aes(xintercept = estimate_norm_DMSO, color = route_group), linetype = 'dashed', size = 0.3, alpha = 0.75) +
#     geom_line(data = predict %>% filter(strains == 'N2', group == 1), aes(x = conc, y = pred_norm_DMSO), color = '#622020', size = 0.3, alpha =0.75) +
#     geom_line(data = predict %>% filter(strains == 'N2', group == 2), aes(x = conc, y = pred_norm_DMSO), color = '#b87333', size = 0.3, alpha =0.75) +
#     geom_line(data = predict %>% filter(strains == 'N2', group == 3), aes(x = conc, y = pred_norm_DMSO), color = '#ebce75', size = 0.3, alpha =0.75) +
#     geom_line(data = predict %>% filter(strains == 'N2', group == 4), aes(x = conc, y = pred_norm_DMSO), color = '#f3830d', size = 0.3, alpha =0.75) +
#     geom_quasirandom(data = well_summary %>% filter(strains == 'N2', treatment %in% c('AlbendazoleSulfoxide', 'Levamisole', 'Ivermectin'),
#                     plate != '20220210-p22-EJG_1187', group == 1), aes(x = conc, y = mean_norm_DMSO), color = '#622020', size = 0.5, width = 0.1, alpha = 0.75) +
#     geom_quasirandom(data = well_summary %>% filter(strains == 'N2', treatment %in% c('AlbendazoleSulfoxide', 'Levamisole', 'Ivermectin'),
#                     plate != '20220210-p22-EJG_1187', group == 2), aes(x = conc, y = mean_norm_DMSO), color = '#b87333', size = 0.5, width = 0.1, alpha = 0.75) +
#     geom_quasirandom(data = well_summary %>% filter(strains == 'N2', treatment %in% c('AlbendazoleSulfoxide', 'Levamisole', 'Ivermectin'),
#                     plate != '20220210-p22-EJG_1187', group == 3), aes(x = conc, y = mean_norm_DMSO), color = '#ebce75', size = 0.5, width = 0.1, alpha = 0.75) +
#     geom_quasirandom(data = well_summary %>% filter(strains == 'N2', treatment %in% c('AlbendazoleSulfoxide', 'Levamisole', 'Ivermectin'),
#                     plate != '20220210-p22-EJG_1187', group == 4), aes(x = conc, y = mean_norm_DMSO), color = '#f3830d', size = 0.5, width = 0.1, alpha = 0.75) +
#     geom_hline(yintercept = 0, color = 'black', size = 0.25) +
#     geom_vline(data = ec50_norm_DMSO_all %>% filter(curve_norm_DMSO == 'N2'), aes(xintercept = estimate_norm_DMSO), linetype = 'dashed', color = 'black', size = 0.5) +
#     geom_line(data = predict_all %>% filter(strains == 'N2'), aes(x = conc, y = pred_norm_DMSO), color = 'black', size = 0.75, alpha =1 ) +
#     
#     scale_x_log10(
#       breaks = c(0.001, 0.01, 0.1, 1, 10, 100),
#       labels = c(0.001, 0.01, 0.1, 1, 10, 100)) +
...
(curve_plot_cuticle <- revised_data %>%
    ...
    geom_vline(data = ec50_norm_DMSO %>% filter(curve_norm_DMSO != 'N2', route == 'Cuticle'),
               aes(xintercept = estimate_norm_DMSO, group = genotype, color = route_group), linetype = 'dashed', size = 0.3, alpha = 0.75) +
    geom_vline(data = N2_ec50_norm_DMSO_bus, aes(xintercept = ec50_average), color = '#fac04e', linetype = 'solid', size = 0.3, alpha = 0.75) +
    geom_vline(data = N2_ec50_norm_DMSO_agm, aes(xintercept = ec50_average), color = '#fac04e', linetype = 'solid', size = 0.3, alpha = 0.75) +
...
(curve_plot_digestive <- revised_data %>%
    ...
    geom_vline(data = ec50_norm_DMSO %>% filter(curve_norm_DMSO != 'N2', route == 'Digestive'),
               aes(xintercept = estimate_norm_DMSO, group = genotype, color = route_group), linetype = 'dashed', size = 0.3, alpha = 0.75) +
    geom_vline(data = N2_ec50_norm_DMSO_eat, aes(xintercept = ec50_average), color = '#fac04e', linetype = 'solid', size = 0.3, alpha = 0.75) +
    geom_vline(data = N2_ec50_norm_DMSO_nhr, aes(xintercept = ec50_average), color = '#fac04e', linetype = 'solid', size = 0.3, alpha = 0.75) +
...
(curve_plots_N2 <- plot_grid(NULL, curve_plot_WT, NULL, align = "h", rel_widths = c(1, 1, 1), nrow = 1))
(curve_plots_mutants_norm <- plot_grid(curve_plot_digestive, curve_plot_cuticle, curve_plot_amphid, align = "h", rel_widths = c(1, 1, 1), nrow = 1))
(curve_plots_combined <- plot_grid(curve_plots_mutants, curve_plots_N2, align = "v", rel_widths = c(1, 1, 1), nrow = 2, ncol = 1))

(curve_plots_combined <- plot_grid(curve_plot_WT, curve_plot_digestive, curve_plot_cuticle, curve_plot_amphid, nrow = 2, ncol = 2))

ggsave(here('plots_EGR/Figure_2/Figure_2.pdf'), curve_plots_mutants_norm, width = 300, height = 90, units = 'mm')
ggsave(here('plots_EGR/Figure_2/Figure_2.png'), curve_plots_combined, width = 300, height = 90, units = 'mm')

ggsave(here('/Users/elenagr/Desktop/Figure_2.png'), curve_plots_mutants_norm, width = 300, height = 90, units = 'mm', bg = "white")

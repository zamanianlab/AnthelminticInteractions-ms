...
(n2_plot <- revised_data %>%
    ...
    geom_line(data = predict_all %>% filter(strains == 'N2'), aes(x = conc, y = pred_norm_DMSO), color = 'black', size = 1, alpha =0.7) + 
    ...
    geom_vline(data = ec50_norm_DMSO_all %>% filter(curve_norm_DMSO == 'N2'), aes(xintercept = estimate_norm_DMSO, group = treatment), size = 1, linetype = 'dashed', color = 'black', alpha = 0.75) +
    scale_x_log10(
...
ggsave(here('/Users/elenagr/Desktop/Fig_1_combined.pdf'), Fig_1_combined, width = 240, height = 80, units = 'mm')
ggsave(here('/Users/elenagr/Desktop/Fig_1_combined.png'), Fig_1_combined, width = 240, height = 80, units = 'mm')
